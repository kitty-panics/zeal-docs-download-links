...
do
    ...
    grep "sanfrancisco" "$SET_TMP_PATH/feeds/$DOCS" | sed 's/^    <url>//g' | sed 's/<\/url>$//g' >> ../official-sanfrancisco.txt
    grep "london"       "$SET_TMP_PATH/feeds/$DOCS" | sed 's/^    <url>//g' | sed 's/<\/url>$//g' >> ../official-london.txt
    grep "newyork"      "$SET_TMP_PATH/feeds/$DOCS" | sed 's/^    <url>//g' | sed 's/<\/url>$//g' >> ../official-newyork.txt
    grep "tokyo"        "$SET_TMP_PATH/feeds/$DOCS" | sed 's/^    <url>//g' | sed 's/<\/url>$//g' >> ../official-tokyo.txt
    grep "frankfurt"    "$SET_TMP_PATH/feeds/$DOCS" | sed 's/^    <url>//g' | sed 's/<\/url>$//g' >> ../official-frankfurt.txt
done
